...
def mutate(creat):
    ...
    new_creat.kernel = creat.kernel + \
        np.random.uniform(-5, 5, size=(kernel_size, kernel_size))
    return new_creat
...
while not done:
    ...
    if cnt == moves-1:
        population = population[np.argsort(
            np.array([fitness(creat) for creat in population]))]
        ...
        for i, _ in enumerate(population):
            parent1 = pop_copy[int(np.random.exponential(5))]
            parent2 = pop_copy[int(np.random.exponential(5))]
